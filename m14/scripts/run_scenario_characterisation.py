"""
Characterize M14 insertion, constant metering, and dynamic schedules (E0).

The full run checks 20 insertion profiles and 12 control profiles. For a
small screening run use --n-insertion 1 --n-profiles 1. Generated trajectories
join the round-0 store unless --no-store is supplied. The E0 gate compares
dynamic schedules with the best constant; the numerical report is evidence,
not an instruction to begin a costly training stage automatically.
"""
...
# Optional characterization constants: the round-0 config may override them under an
...
def main() -> None:
    ...
    ap.add_argument("--out", default="runs/study/m14/e0.json")
    ap.add_argument("--study", default="m14_e0")
    ...
    results = run_jobs(jobs, env_cfg, workers=args.workers,
                       network_root=str(PROJECT_ROOT / out_cfg.get("network_dir", "data/networks/dataset")))
