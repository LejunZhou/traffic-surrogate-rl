"""
Evaluate manifest arms on held-out test and OOD demand profiles in SUMO.

Final evaluation is excluded from the training SUMO budget. Existing output
JSONL files are validated and reused when their request fingerprint matches. Test rollouts are saved beside their JSONL reports.
Example: python scripts/run_final_evaluation.py
         --arms runs/study/m14/arms.json --workers 8 --study m14_final
"""
    ...
    for set_name in args.sets:
        ...
        for arm in manifest["arms"]:
            ...
            for p in arm["points"]:
                ...
                # (per-point `ensemble_dir`); otherwise use the study config statistics
                if p.get("ensemble_dir"):
                    extra["ensemble_dir"] = p["ensemble_dir"]
                env_cfg = sumo_env_config(PROJECT_ROOT, network_dir=f"data/networks/final_{set_name}", extra=extra)
                ...
                res = evaluate_on_profiles([p["policy"]], profiles, env_cfg, out, workers=args.workers, purpose=purpose, study=args.study,
                                           project_root=PROJECT_ROOT, network_root=f"{PROJECT_ROOT}/data/networks/final_{set_name}",
