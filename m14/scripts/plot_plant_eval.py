"""
Field-level figures for a plant-model ensemble on a held-out split.

  PYTHONPATH=src python scripts/plot_plant_eval.py \\
      --ensemble runs/deeponet/round0 --split test \\
      --out runs/figures/deeponet
...
REGIME_SLOT = {"store_flush": 0, "constant": 1, "alinea": 2, "random_signal": 3, "feedforward": 5,
               "alinea_wide": 2, "alinea_dither": 6}
...
def main() -> None:
    ...
    ap.add_argument("--out", default="runs/figures/deeponet")
    args = ap.parse_args()
    ens = DeepONetEnsemble.load(PROJECT_ROOT / args.ensemble)
    store = Path(ens.manifest.get("store_dir", "data/round0"))
    ...
    def cells(preferred):
        ...
        for g, bd in preferred:
            if g in groups_present and any(r["group"] == g and bool(r["breakdown_true"]) == bd for r in rows):
                out.append((g, bd))
        for g in groups_present:                       # regimes not in the preferred list
            if all(c[0] != g for c in out):
                bd = any(r["group"] == g and r["breakdown_true"] for r in rows)
                out.append((g, bd))
        return out
    reps = _pick_representatives(rows, cells([("store_flush", True), ("alinea", True), ("random_signal", True), ("alinea_dither", False)])[:4])
    reps6 = _pick_representatives(rows, cells([("constant", True), ("store_flush", True), ("feedforward", True), ("alinea", True),
                                               ("random_signal", True), ("alinea_dither", False)])[:7])
