"""
Shared reward accounting and M14 total-time-spent objective.
...
UNIT = RewardWeights(form="three_term", delta=1.0, beta=1.0, gamma=1.0, q_ref=3000.0, queue_norm=400.0, sigma_ref=6.0)
...
def test_weights_scale_terms_independently():
    ...
    heavy = reward_terms(
        rho, 100.0, 1500.0,
        RewardWeights(form="three_term", delta=3.0, beta=1.0, gamma=1.0, q_ref=3000.0, queue_norm=400.0, sigma_ref=6.0),
    )
...
def test_none_outflow_requires_delta_zero():
    ...
    two_term = RewardWeights(form="three_term", delta=0.0, beta=1.0, gamma=1.0, q_ref=3000.0, queue_norm=400.0, sigma_ref=6.0)
...
def test_from_config_rejects_obsolete_reward_keys():
...
def test_from_config_reads_new_keys_and_ignores_env_keys():
    w = RewardWeights.from_config(
        {"form": "three_term", "delta": 2.0, "beta": 0.5, "gamma": 0.8, "q_ref": 2500.0, "queue_norm": 300.0,
...
def test_input_validation():
    ...
    with pytest.raises(ValueError):
        RewardWeights(form="three_term", delta=-1.0)


# Offered-demand reference, terminal cost and total-time-spent accounting.
...
def test_terminal_queue_cost_only_on_terminal_step():
    ...
    w = RewardWeights(form="three_term", delta=1.0, beta=1.0, gamma=0.0, q_ref=2000.0, queue_norm=100.0, terminal_queue_weight=2.0)
