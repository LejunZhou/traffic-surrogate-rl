...
def test_integral_sign():
    c = PIALINEAController(ki=40.0, rho_set=30.0, u_init=0.5, discharge_vph=1200.0)
    ...
    assert u_low == pytest.approx(0.5 + 40.0 * 10.0 / 1200.0)
    assert u_high == pytest.approx(0.5 - 40.0 * 10.0 / 1200.0)
...
def test_anti_windup():
    ...
    u = c(obs_vec(40.0))[0]
    assert u == pytest.approx(1.0 - 100.0 * 10.0 / 1200.0)
...
def test_pi_damps_rising_density():
    ...
    assert u_i == pytest.approx(0.5 + 40.0 * (5.0 + 1.0) / 1200.0)
    assert u_pi == pytest.approx(u_i - 80.0 * 4.0 / 1200.0)
...
def test_queue_override_reads_ramp_demand_from_obs():
    ...
    assert u == pytest.approx(600.0 / 1200.0)
