...
requires_sumo = pytest.mark.skipif(shutil.which("sumo") is None, reason="sumo binary not available")


def _env(tmp_path):
    from rl.sumo_env_wrapper import SumoEnv

    cfg = dict(load_config(str(PROJECT_ROOT / "configs/dataset.yaml"))["env"])
    cfg["project_root"] = str(PROJECT_ROOT)
    cfg["network_dir"] = str(tmp_path / "network")
    return SumoEnv(cfg)


@requires_sumo
def test_m14_geometry_and_stop_line(tmp_path):
    env = _env(tmp_path)
...
@requires_sumo
def test_m14_no_cap_queue_grows(tmp_path):
    env = _env(tmp_path)
