...
def rollout_episode(env, controller, reset_options: dict | None = None, deterministic: bool = True) -> dict:
    ...
    # Preserve the environment's simulation-step queue statistics separately
    # from max_queue, which is computed from control-interval samples above.
    for key in ("episode_queue_mean", "episode_queue_max"):
        if key in last_info:
            metrics[key] = float(last_info[key])
