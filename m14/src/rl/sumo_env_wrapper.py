"""M14 Gymnasium environment backed by live SUMO through TraCI.

At each 30-second decision, the controller meters the virtual ramp queue;
confirmed departures reduce that queue. Interval detector aggregates become
the next observation. M14 uses offered demand profiles, occupancy density at
19 stations, through-lane density at the merge, and a TTS reward. The current
observation contains normalized densities, mainline/ramp demands, time and
queue, with optional configured demand look-ahead.

The environment and network helpers retain configurable geometry and density
estimators for tests. configs/scenario.yaml defines the M14 v3b experiment.
"""
    ...
    def __init__(self, config: dict) -> None:
        ...
        Args:
            config: SUMO RL environment config. Usually this is the ``env``
                    section from configs/ppo.yaml.
        """
        super().__init__()
        self.env_config = copy.deepcopy(config)
        self.project_root = Path(
            self.env_config.get("project_root", Path(__file__).resolve().parents[2])
        ).resolve()
        ...
        self.network_dir = self._resolve_path(
            self.env_config.get(
                "network_dir",
                self.sumo_config.get("output", {}).get("network_dir", "data/networks/rl"),
            )
        )
