"""Batched M14 PPO environment backed by a causal GRU DeepONet ensemble.

Each slot owns a demand profile, analytic ramp queue, and inflow history.
The queue converts the action to released ramp flow; DeepONet returns the
interval density and exit flow. Training samples one member per episode;
mean-member evaluation and optional pessimistic rewards are also supported.
M14 has no finite storage cap and has physical discharge 1200 veh/h.
"""
...
class SurrogateVecEnv(VecEnv):
    def __init__(self, config: dict, plant=None) -> None:
        ...
        cfg = self.env_config
        self.project_root = Path(cfg.get("project_root", Path(__file__).resolve().parents[2])).resolve()
