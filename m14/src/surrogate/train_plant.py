"""Train and fine-tune one M14 GRU DeepONet ensemble member.

The loss is shock-band-weighted density MSE plus normalized exit-flow MSE.
Members bootstrap the round-zero split and retain it during aggregation.
AdamW and a cosine schedule train the model; checkpoint selection uses the
sum of physical-unit density and exit-flow validation relative-L2 errors.
"""
...
def train_member(config: dict, member: int, bootstrap_seed: int | None, out_dir: Path, resume: Path | None = None,
    ...
    root = Path(project_root or config.get("project_root", Path(__file__).resolve().parents[2]))
...
def main() -> None:
    ...
    ap.add_argument("--max-train-files", type=int, default=None, help="cap round-zero training files for a small run")
    ap.add_argument("--train-types", nargs="*", default=None, help="keep only these round-zero controller types")
    args = ap.parse_args()
