...
FAMILY = ROOT / "configs" / "profiles" / "family_v1.yaml"
...
def test_family_bounds_and_drain_tail():
    ...
    for i in range(300):
        ...
        assert p.ramp_blocks.max() <= 1000.0 + 1e-3
...
def test_family_v2_constraints():
    """M14 family v2: everything over by minute 45, ramp surge never above 800 veh/h."""
    fam = ProfileFamily.load(ROOT / "configs" / "profiles" / "family_v2.yaml")
    for i in range(500):
        p = fam.sample_by_key("train", i)
    ...
    for i in range(30):
        ...
        assert p.ramp_blocks.max() <= 800.0 + 1e-3
    # v1 behaviour unchanged (keys absent)
    v1 = ProfileFamily.load(FAMILY)
    assert v1.sample_by_key("train", 7).ramp_blocks.max() <= 1000.0 + 1e-3
