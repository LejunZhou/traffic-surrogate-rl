"""Plant-model DeepONet v2: causality, shape contracts, ensemble loading,
...
@pytest.mark.parametrize("btype", ["gru", "causal_conv"])
def test_causality(btype):
    torch.manual_seed(0)
    m = build_plant_model({"branch": {"type": btype, "channels": 32, "hidden": 32, "latent_dim": 64}, "trunk": {"hidden_dim": 64, "latent_dim": 64}})
    x = torch.randn(2, 2, K)
...
def test_forward_shapes_and_padded_branch():
    ...
    mp = build_plant_model({"branch": {"type": "mlp_padded", "hidden": 32, "latent_dim": 32}, "trunk": {"hidden_dim": 32, "latent_dim": 32}}, K=K)
    assert mp.branch(torch.randn(3, 2, K)).shape == (3, 1, 32)
    rho, q = mp(torch.randn(3, 2, K), torch.randint(0, K, (3, 5)), torch.rand(3, 5, 2))
...
def _tiny_cfg(store: Path) -> dict:
    return {"project_root": str(ROOT),
            "data": {"store_dir": str(store), "sumo_config": "configs/sumo/scenario_v2.yaml", "n_query_points": 64, "mode": "causal",
...
def test_dataset_views_and_bootstrap(tiny_ensemble):
    ...
    dsp = PlantRolloutDataset(store, split["train"], norm, n_query_points=16, mode="padded", padded_views_per_rollout=3)
    fn, m = dsp.views[0]
    it = dsp[0]
    assert float(it["branch"][:, m:].abs().sum()) == 0.0 and int(it["query_k"].max()) < m and float(it["mask_q"].sum()) == m
...
def test_sumo_and_surrogate_env_parity(tiny_ensemble):
    ...
    common = {"project_root": str(ROOT), "density_mean": 20.0, "density_std": 16.0, "queue_norm_scale": 100.0,
              "observation": {"lookahead_steps": 2}, "reward": reward, "profiles": {"set": "configs/profiles/val.json"}}
    sumo = SumoEnv({**common, "sumo_config": "configs/sumo/scenario_v2.yaml", "network_dir": "data/raw/_test_parity_net"})
