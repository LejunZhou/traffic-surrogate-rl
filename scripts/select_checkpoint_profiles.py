"""
Post-hoc checkpoint selection for a PPO run evaluated on the V profiles
(run-7 recipe on profiles): rank the run's checkpoints by the deterministic
V evaluation stored in eval/evaluations.npz, require no catastrophic episode,
and copy the winner to <run>/best_model_selected.zip. For SUMO runs the
evaluation episodes are already in the ledger; nothing new is simulated.

  PYTHONPATH=src python scripts/select_checkpoint_profiles.py --run runs/study/demo/direct_ppo_200ee_s0 [--catastrophic -250]
"""
