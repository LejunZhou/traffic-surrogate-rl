"""
E6 — ALINEA / PI-ALINEA tuning on the validation profiles (two-stage protocol
of _plans/alinea_baseline_plan.md adapted to the profile family).

Stage 1: detector {11, 12, 13} x set-point {26, 30, 34, 38} x K_I {10, 20, 35}
         (36 candidates) on a 6-profile subset of V (216 EE).
Stage 2: the top 4 of stage 1, plus PI variants (kp 4), on all 18 V profiles
         (~144 EE). Winner = best mean return without catastrophic episodes.
Constant-u reference: u in {0.0, 0.1, ..., 1.0} on all 18 V profiles (198 EE).

  PYTHONPATH=src python scripts/tune_alinea_profiles.py --workers 8 [--study m11_alinea]
Writes _progress/m11_alinea_tuning.json and the ledger (purpose tuning).
"""
...
def main() -> None:
    ...
    ap.add_argument("--study", default="m11_alinea")
    ...
    ap.add_argument("--out", default="_progress/m11_alinea_tuning.json")
    args = ap.parse_args()
    val = load_set("val", PROJECT_ROOT)
    env_cfg = sumo_env_config(PROJECT_ROOT, network_dir="data/raw/rl_network_tune")
    ...
    res1 = evaluate_on_profiles(cands, subset, env_cfg, out_dir / "stage1.jsonl", workers=args.workers, purpose="tuning",
                                study=args.study, project_root=PROJECT_ROOT, network_root=str(PROJECT_ROOT / "data/raw/rl_network_tune"), quiet=True)
