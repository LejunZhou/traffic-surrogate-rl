"""
Evaluate a plant-model ensemble on held-out rollouts, per regime (M9, §5.5).

  PYTHONPATH=src python scripts/eval_surrogate_regimes.py \\
      --ensemble runs/surrogate/plant_v2_round0 [--store data/plant_v2/round0] [--split val|test]
      [--files a.npz b.npz] [--reward-config configs/rl/ppo_common.yaml] [--out <dir>]

Prints the per-regime table and the round-0 acceptance gate (return-prediction
error <= 10 %, false breakdown rate <= 10 %, calibration slope in [0.5, 2]).
"""
...
def main() -> None:
    ...
    store = Path(args.store or ens.manifest.get("store_dir", "data/plant_v2/round0"))
