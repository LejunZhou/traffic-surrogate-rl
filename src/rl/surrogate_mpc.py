"""
Surrogate-MPC (M11, draft_pipeline.md §9): gradient-based model predictive
...
and the objective is the three-term reward summed over the horizon plus a
...
class SurrogateMPC:
    def __init__(self, ensemble, weights: RewardWeights, horizon: int = 20, iters: int = 30, lr: float = 0.1,
                 discharge_vph: float = 1600.0, terminal_weight: float = 0.5, offset: bool = True, offset_decay: float = 0.9,
    ...
    def from_spec(cls, spec: str, env=None, env_cfg: dict | None = None) -> "SurrogateMPC":
        ...
        env_cfg = env_cfg or {}
        root = Path(env_cfg.get("project_root", Path.cwd()))
        ...
        if head == "onestep_mpc":
            ...
            ens = OneStepEnsemble.load(path, members=members)
            raise NotImplementedError("gradient MPC through the one-step model is not part of the pipeline")
        ...
        discharge = float(env.ramp_discharge_vph) if env is not None else float(env_cfg.get("ramp_discharge_vph", 1600.0))
