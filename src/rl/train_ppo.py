...
M10 additions (draft_pipeline.md §7.4): one common config plus an
environment overlay (`--config configs/rl/ppo_common.yaml --overlay
configs/rl/env_surrogate.yaml`), the batched SurrogateVecEnv v2
(env.ensemble_dir), profile-cycling evaluation on the frozen V set for both
environments, a checkpoint at every evaluation pass, and `--init-policy`
warm starts (aggregation rounds, SUMO fine-tuning).
...
def main() -> None:
    ...
    parser.add_argument("--overlay", action="append", default=[],
                        help="YAML overlay(s) deep-merged onto --config (e.g. configs/rl/env_surrogate.yaml)")
